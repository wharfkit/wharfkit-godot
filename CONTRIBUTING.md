...
This repo is a Cargo workspace. The core `wharfkit-godot` cdylib lives at the root; the `wharfkit-godot-plugin-sdk` crate (the typed Rust API plugin authors use to build wallet plugins, transact plugins, and UI renderers without taking a Rust dep on the core cdylib) lives at `crates/wharfkit-godot-plugin-sdk/`.
...
└── wharfkit-godot/
    └── crates/wharfkit-godot-plugin-sdk/
```
