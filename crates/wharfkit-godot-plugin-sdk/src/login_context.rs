...
impl LoginContextView {
    ...
    pub fn chain_id(&self) -> Option<String> {
        match &self.inner {
            Inner::Dict(d) => {
                dict::dict_dict(d, "chain").and_then(|chain| dict::dict_string(&chain, "id"))
            }
            Inner::Object(o) => o
    ...
    pub fn chain_url(&self) -> Option<String> {
        match &self.inner {
            Inner::Dict(d) => {
                dict::dict_dict(d, "chain").and_then(|chain| dict::dict_string(&chain, "url"))
            }
            Inner::Object(o) => o
    ...
    pub fn app_name(&self) -> Option<String> {
        match &self.inner {
            Inner::Dict(d) => dict::dict_string(d, "app_name"),
            Inner::Object(o) => o
                .get(&StringName::from("app_name"))
                .try_to::<GString>()
                .ok()
                .map(|s| s.to_string()),
        }
    ...
    pub fn arbitrary(&self) -> Dictionary {
        match &self.inner {
            Inner::Dict(d) => dict::dict_dict(d, "arbitrary").unwrap_or_default(),
            Inner::Object(o) => o
    ...
    pub fn as_object(&self) -> &Gd<godot::classes::Object> {
        self.try_as_object()
            .expect("LoginContextView is not Object-backed")
    }
